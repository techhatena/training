...
export const getValidationPresetsByType = (componentType: string): { key: ValidationPresetKey; label: string }[] => {
    const presetKeys: ValidationPresetKey[] = (() => {
    ...
    return presetKeys.map(key => ({
        key,
        label: VALIDATION_PRESETS[key].label
    }));
};